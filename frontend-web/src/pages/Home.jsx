    ...
    <div className="p-2 sm:p-4 bg-gradient-to-br from-blue-50 to-blue-200 min-h-screen relative">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4 gap-2">
        ...
        <button
          ...
          className="bg-gradient-to-br from-red-600 via-red-500 to-red-800 text-white px-4 py-2 rounded-lg  hover:scale-105 active:scale-95 transition"
        >
          Logout
        </button>
      </div>
        ...
        <div className="flex flex-wrap gap-2 justify-center sm:justify-start">
          {['Recent', 'Oldest', 'Credit'].map((type) => (
            <button
              ...
              className={`px-4 py-2 rounded-lg text-sm  border transition font-medium ${sort === type.toLowerCase() ? 'bg-blue-600 text-white' : 'bg-white text-gray-700 hover:bg-blue-100'}`}
            >
              {type}
            </button>
          ))}
      ...
      {/* Customer List */}
      <div className="bg-white rounded-xl  divide-y overflow-hidden mb-24">
        {customers.length === 0 && (
      ...
      <div className="fixed bottom-0 left-0 w-full bg-gradient-to-t from-blue-100 via-white to-transparent px-4 py-3 z-20 border-t border-blue-200">
        <div className="flex justify-between gap-2">
          <button
            ...
            className="flex-1 bg-green-600 hover:bg-green-700 active:scale-95 text-white py-2 rounded-lg  font-semibold transition"
          >
            + Sale
          </button>
          <button
            onClick={() => setShowProductPopup(true)}
            className="flex-1 bg-blue-600 hover:bg-blue-700 active:scale-95 text-white py-2 rounded-lg font-semibold transition"
          >
            + Product
          </button>
          <button
            onClick={() => setShowPopup(true)}
            className="flex-1 bg-indigo-600 hover:bg-indigo-700 active:scale-95 text-white py-2 rounded-lg font-semibold transition"
          >
            + Customer
          </button>
        </div>
