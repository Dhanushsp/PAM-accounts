...
interface AddPayablesPopupProps {
  token: string;
  onClose: () => void;
  onPayablesAdded?: () => void;
}

export default function AddPayablesPopup({ token, onClose, onPayablesAdded }: AddPayablesPopupProps) {
  const [selectedType, setSelectedType] = useState<any>(null);
  const [newType, setNewType] = useState('');
  const [amount, setAmount] = useState('');
  const [date, setDate] = useState(new Date().toISOString().split('T')[0]);
  const [payableTypes, setPayableTypes] = useState<any[]>([]);
  ...
  const handleAddNewType = async () => {
    ...
    try {
      ...
      setPayableTypes(prev => [...prev, createdType]);
      setSelectedType(createdType);
      setNewType('');
      setShowNewTypeInput(false);
    } catch (error: any) {
      console.error('Error adding payable type:', error);
      Alert.alert('Error', error.response?.data?.message || 'Failed to add payable type');
    }
  };

  const handleSubmit = async () => {
    if (!selectedType) {
      Alert.alert('Error', 'Please select a payable type');
      return;
    }
    ...
    try {
      ...
      console.error('Error adding payable entry:', error);
      Alert.alert('Error', error.response?.data?.message || 'Failed to add payable entry');
    } finally {
      setLoading(false);
    }
  };
    ...
    <View style={styles.overlay}>
      <View style={[
        ...
        <View style={[
          ...
          <Text style={styles.title}>Add Payable</Text>
          ...
          >
            ...
            <View style={styles.section}>
              <Text style={styles.sectionTitle}>Payable Type</Text>
              
              {!showNewTypeInput ? (
                <View style={styles.payableTypesSection}>
                  <View style={styles.payableTypesHeader}>
                    <Text style={styles.payableTypesLabel}>Select Payable Type:</Text>
                    <TouchableOpacity
                  ...
                  {/* Horizontal Payable Type Buttons */}
