...
export default function PayablesTab({ token }: PayablesTabProps) {
  ...
  const fetchPayableTypes = async () => {
    try {
      ...
      Alert.alert('Error', 'Failed to fetch payables data');
    } finally {
      setLoading(false);
    }
  };
  ...
  const getTotalPayables = () => {
    return payableTypes.reduce((total, type) => total + type.totalAmount, 0);
  };

  const getFilteredEntries = (entries: PayableEntry[]) => {
  ...
  const getFilteredTypes = () => {
    ...
    return payableTypes.filter(type => type.name === filterType);
  };

  const handleTypePress = (type: PayableType) => {
  ...
  return (
    <View style={styles.container}>
      {/* Total Payables */}
      <View style={styles.totalContainer}>
        <Text style={styles.totalLabel}>Total Payables</Text>
        <Text style={styles.totalAmount}>₹{getTotalPayables().toLocaleString()}</Text>
      </View>
      ...
      {loading ? (
        ...
        <View style={styles.centerContainer}>
          <Text style={styles.emptyText}>No payable types found</Text>
        </View>
      ) : (
